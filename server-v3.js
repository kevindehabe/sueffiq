...
function publicState(room, forId) {
  return {
    ...
    current: currentPublic(room, forId), result: room.phase === 'results' ? room.lastResult : null, version: '3.0.0',
  };
}

function broadcast(room) {
  for (const id of room.order) { const p = room.players[id]; if (p?.connected) send(p.ws, { t: 'state', s: publicState(room, id) }); }
}
...
function finishRound(room, reason = 'complete') {
  ...
  if (reason === 'timeout' && !['person', 'bild', 'wahrheit', 'pflicht'].includes(cur.type)) {
...
async function resolveWikiImage(person) {
  ...
  try {
    const response = await fetch(url, { signal: ctrl.signal, headers: { 'user-agent': 'SueffIQ/3.0 (https://github.com/kevindehabe/sueffiq)' } });
...
async function startRound(room) {
  ...
  if (type === 'person') {
    cur.text = 'Wer ist die gesuchte Person?'; cur.person = pick(room, type); cur.hintIndex = 0; cur.guessFeed = []; cur.personCorrect = {}; cur.personNear = {};
  }
  if (type === 'bild') {
...
function answerCurrent(room, me, value) {
  const cur = room.current;
  if (!cur || room.phase !== 'question' || ['person', 'bild'].includes(cur.type)) return;
...
function guessCurrent(room, me, rawGuess) {
  const cur = room.current;
  if (!cur || room.phase !== 'question' || !['person', 'bild'].includes(cur.type) || cur.personCorrect[me]) return;
  ...
  if (match.status === 'correct') {
    ...
    cur.personNear[me] = false; cur.guessFeed.push({ name: displayName(room, me), guess, hint: stage });
...
const indexFile = path.join(__dirname, 'public', 'v3.html');
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/healthz') {
    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
    res.end(JSON.stringify({ ok: true, rooms: rooms.size, version: '3.0.0' })); return;
  }
  if (req.url === '/robots.txt') { res.writeHead(200, { 'content-type': 'text/plain; charset=utf-8' }); res.end('User-agent: *\nDisallow:\n'); return; }
  if (req.url !== '/' && req.url !== '/index.html' && req.url !== '/v3.html') { res.writeHead(302, { location: '/' }); res.end(); return; }
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.t === 'create') {
      room = createRoom(); me = uid(); room.players[me] = { name: cleanName(msg.name, 'Host'), drinks: 0, connected: true, ws };
      room.order.push(me); room.hostId = me; send(ws, { t: 'joined', code: room.code, id: me }); return broadcast(room);
    }
    if (msg.t === 'join') {
      const code = String(msg.code || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5); const target = rooms.get(code);
      if (!target) return error('Diesen Spielcode gibt es nicht.'); room = target; me = uid();
      room.players[me] = { name: cleanName(msg.name), drinks: 0, connected: true, ws }; room.order.push(me);
    ...
    if (msg.t === 'start' && isHost && (room.phase === 'lobby' || room.phase === 'end')) {
      if (!connectedIds(room).length) return error('Niemand ist im Raum.'); startRound(room).catch(() => error('Runde konnte nicht gestartet werden.')); return;
    }
    ...
    if (msg.t === 'host' && isHost && room.players[msg.id]?.connected) { room.hostId = msg.id; return broadcast(room); }
  ...
  ws.on('close', () => {
    ...
    if (room.hostId === me) { const next = connectedIds(room)[0]; if (next) room.hostId = next; }
    if (room.phase === 'question' && allAnswered(room)) finishRound(room, 'complete'); else broadcast(room);
    setTimeout(() => { if (!rooms.has(room.code)) return; if (!connectedIds(room).length) { clearTimers(room); rooms.delete(room.code); } }, 30 * 60 * 1000);
  });
});

server.listen(PORT, '0.0.0.0', () => {
  const ips = Object.values(os.networkInterfaces()).flat().filter((i) => i && i.family === 'IPv4' && !i.internal).map((i) => i.address);
  console.log(`SüffIQ v3 läuft auf http://localhost:${PORT}`);
