...
const WEIGHTS = {
  nie: 3, wahl: 3, schaetz: 3, oder: 2, trivia: 4, wahrheit: 3,
  pflicht: 2, person: 3, bild: 3, song: 3, mehrheit: 2, skala: 2,
};
const ROUND_SECONDS = {
...
function publicState(room, forId) {
  return {
    ...
    current: currentPublic(room, forId), result: room.phase === 'results' ? room.lastResult : null, version: '4.0.0',
  };
}
...
function matchSongGuess(song, rawGuess) {
  ...
  for (const title of titles) {
    ...
    if (guess.length >= 5 && (guess.includes(title) || title.includes(guess)) && Math.min(guess.length, title.length) / Math.max(guess.length, title.length) >= 0.7) {
      return { status: 'correct' };
    }
    if (similarity(guess, title) >= 0.84) return { status: 'correct' };
  }
  if (artist && (guess === artist || similarity(guess, artist) >= 0.84)) return { status: 'artist' };
...
async function resolveWikiImage(person) {
  ...
  try {
    const response = await fetch(url, { signal: ctrl.signal, headers: { 'user-agent': 'SueffIQ/4.0 (https://github.com/kevindehabe/sueffiq)' } });
...
const indexFile = path.join(__dirname, 'public', 'v4.html');
const serverHttp = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/healthz') {
    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
    res.end(JSON.stringify({ ok: true, rooms: rooms.size, version: '4.0.0' })); return;
  }
  if (req.url === '/robots.txt') { res.writeHead(200, { 'content-type': 'text/plain; charset=utf-8' }); res.end('User-agent: *\nDisallow:\n'); return; }
  if (req.url !== '/' && req.url !== '/index.html' && req.url !== '/v4.html') { res.writeHead(302, { location: '/' }); res.end(); return; }
  res.writeHead(200, {
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.t === 'join') {
      ...
      room.order.push(me); send(ws, { t: 'joined', code: room.code, id: me }); return broadcast(room);
    }
    if (msg.t === 'rejoin') {
    ...
    if (msg.t === 'start' && isHost && (room.phase === 'lobby' || room.phase === 'end')) {
      ...
      startRound(room).catch(() => error('Runde konnte nicht gestartet werden.')); return;
    }
  ...
  ws.on('close', () => {
    ...
    setTimeout(() => {
      ...
      if (!connectedIds(room).length) { clearTimers(room); rooms.delete(room.code); }
    }, 30 * 60 * 1000);
  });
});
...
serverHttp.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`SüffIQ v4 läuft auf http://localhost:${PORT}`);
