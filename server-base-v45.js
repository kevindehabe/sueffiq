...
function currentPublic(room, forId) {
  ...
  if (cur.type === 'person' || cur.type === 'bild') {
    ...
    base.yourStatus = cur.personCorrect[forId] ? { status: 'correct', hint: cur.personCorrect[forId] } : cur.personNear[forId] ? { status: 'near' } : null;
  }
  if (cur.type === 'person') {
  ...
  if (cur.type === 'bild') {
    base.imageUrl = cur.imageUrl; base.imageSource = cur.imageSource; base.blurStep = cur.blurStep; base.blurSteps = 5;
  }
  if (cur.type === 'song') {
    ...
    base.guessFeed = cur.guessFeed.slice(-20);
    base.yourStatus = cur.songCorrect[forId] ? { status: 'correct', hint: cur.songCorrect[forId] } : cur.songNear[forId] ? { status: 'near' } : null;
...
function matchSongGuess(song, rawGuess) {
  ...
  const artistParts = String(song.artist || '').split(/\s+(?:feat\.?|ft\.?)\s+|\s*&\s*|\s+x\s+/i).map(normalizeText).filter(Boolean);
...
function finishRound(room, reason = 'complete') {
  ...
  if (reason === 'timeout' && !['person', 'bild', 'song', 'wahrheit', 'pflicht'].includes(cur.type)) {
    const late = ids.filter((id) => cur.answers[id] === undefined); late.forEach((id) => { sipMap[id] = Math.max(sipMap[id] || 0, 1); });
    if (late.length) result.lines.push(`Zu langsam: ${late.map((id) => displayName(room, id)).join(', ')}.`);
  }
  result.drinkers = giveSips(room, sipMap); room.lastResult = result; room.phase = 'results'; broadcast(room);
}
function allAnswered(room) {
  const cur = room.current; if (!cur) return false;
  const ids = connectedIds(room); if (!ids.length) return false;
  ...
  if (cur.type === 'song') { const participants = songPlayers(room); return participants.length > 0 && participants.every((id) => !!cur.songCorrect[id]); }
  return ids.every((id) => cur.answers[id] !== undefined);
}
async function resolveWikiImage(person) {
  const key = `${person.lang || 'de'}:${person.page}`; if (wikiCache.has(key)) return wikiCache.get(key);
  const lang = person.lang || 'de'; const url = `https://${lang}.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(person.page)}`;
  ...
  try {
    ...
    const data = await response.json(); const imageUrl = data.thumbnail?.source || data.originalimage?.source; if (!imageUrl) throw new Error('Kein Bild');
    const out = { imageUrl, source: data.content_urls?.desktop?.page || `https://${lang}.wikipedia.org/wiki/${person.page}` }; wikiCache.set(key, out); return out;
  } finally { clearTimeout(timer); }
}
function scheduleRound(room) {
  const cur = room.current; const round = room.round; const sec = cur.total;
  if (cur.type === 'person') {
    const step = Math.floor((sec * 1000) / cur.person.hints.length);
    for (let hint = 1; hint < cur.person.hints.length; hint += 1) addTimer(room, () => {
      if (room.phase !== 'question' || room.round !== round || room.current.type !== 'person') return;
      room.current.hintIndex = Math.max(room.current.hintIndex, hint); broadcast(room);
    }, step * hint);
  }
  if (cur.type === 'bild') {
    const step = 4000;
    for (let i = 1; i < 5; i += 1) addTimer(room, () => {
      if (room.phase !== 'question' || room.round !== round || room.current.type !== 'bild') return;
      room.current.blurStep = Math.max(room.current.blurStep, i); broadcast(room);
    }, step * i);
  }
...
async function startRound(room) {
  ...
  if (type === 'person') { cur.text = 'Wer ist die gesuchte Person?'; cur.person = pick(room, type); cur.hintIndex = 0; cur.guessFeed = []; cur.personCorrect = {}; cur.personNear = {}; }
  if (type === 'bild') {
    cur.text = 'Wer ist das? Das Bild wird immer schärfer.'; cur.guessFeed = []; cur.personCorrect = {}; cur.personNear = {}; cur.blurStep = 0;
    let person = null; let resolved = null;
    for (let attempt = 0; attempt < 4; attempt += 1) { person = pick(room, type); try { resolved = await resolveWikiImage(person); break; } catch {} }
    if (!resolved) {
...
function guessCurrent(room, me, rawGuess) {
  ...
  if (cur.type === 'song') {
    ...
    const guess = String(rawGuess || '').trim().slice(0, 64); if (guess.length < 2) return;
    const match = matchSongGuess(cur.song, guess); const elapsedMs = cur.songStartedAt ? Math.max(0, Date.now() - cur.songStartedAt) : 0;
    const stage = elapsedMs <= 5000 ? 1 : elapsedMs <= 12000 ? 2 : elapsedMs <= 22000 ? 3 : elapsedMs <= 35000 ? 4 : 5;
    if (match.status === 'correct') { cur.songCorrect[me] = stage; cur.songNear[me] = false; send(room.players[me].ws, { t: 'guessFeedback', status: 'correct', m: 'Richtig! Dein Tipp bleibt geheim.' }); }
    else if (match.status === 'artist') { cur.songNear[me] = true; send(room.players[me].ws, { t: 'guessFeedback', status: 'near', m: 'Interpret stimmt – jetzt fehlt noch der Songtitel.' }); }
    else if (match.status === 'near') { cur.songNear[me] = true; send(room.players[me].ws, { t: 'guessFeedback', status: 'near', m: 'Sehr nah dran – bleibt privat.' }); }
    else { cur.songNear[me] = false; cur.guessFeed.push({ name: displayName(room, me), guess, hint: stage }); if (cur.guessFeed.length > 40) cur.guessFeed.shift(); send(room.players[me].ws, { t: 'guessFeedback', status: 'wrong', m: 'Falsch – dieser Tipp ist für alle sichtbar.' }); }
  } else {
    if (cur.personCorrect[me]) return;
    const guess = String(rawGuess || '').trim().slice(0, 48); if (guess.length < 2) return;
    const match = matchPersonGuess(cur.person, guess); const stage = cur.type === 'person' ? cur.hintIndex + 1 : cur.blurStep + 1;
    if (match.status === 'correct') { cur.personCorrect[me] = stage; cur.personNear[me] = false; send(room.players[me].ws, { t: 'guessFeedback', status: 'correct', m: 'Richtig! Dein Tipp bleibt geheim.' }); }
    else if (match.status === 'near') { cur.personNear[me] = true; send(room.players[me].ws, { t: 'guessFeedback', status: 'near', m: 'Sehr nah dran – bleibt privat.' }); }
    else { cur.personNear[me] = false; cur.guessFeed.push({ name: displayName(room, me), guess, hint: stage }); if (cur.guessFeed.length > 40) cur.guessFeed.shift(); send(room.players[me].ws, { t: 'guessFeedback', status: 'wrong', m: 'Falsch – dieser Tipp ist für alle sichtbar.' }); }
  }
  if (allAnswered(room)) finishRound(room, 'complete'); else broadcast(room);
}
function setSongStage(room, me, value) {
  const cur = room.current; if (!cur || room.phase !== 'question' || cur.type !== 'song' || room.hostId !== me) return;
  const stage = clamp(Number(value) || 0, 0, SONG_SNIPPETS.length - 1); cur.songStage = Math.max(cur.songStage, stage); broadcast(room);
}
function retargetTruthOrDare(room, oldId) {
  ...
  const ids = connectedIds(room); if (!ids.length) return false;
  room.current.target = rand(ids); room.current.answers = {}; broadcast(room); return true;
}
function leaveRoom(room, me) {
  if (!room || !room.players[me]) return;
  room.players[me].connected = false; room.players[me].ws = null;
  if (room.hostId === me) room.hostId = connectedIds(room)[0] || null;
  if (retargetTruthOrDare(room, me)) return;
  if (room.phase === 'question' && allAnswered(room)) finishRound(room, 'complete'); else broadcast(room);
}

function replaceRequired(source, needle, replacement, label) { if (!source.includes(needle)) throw new Error(`Frontend-Patch fehlt: ${label}`); return source.replace(needle, replacement); }
function buildHtml() {
  let html = fs.readFileSync(path.join(__dirname, 'public', 'v4.html'), 'utf8');
  html = html.replace('SüffIQ v4.0', `SüffIQ v${VERSION}`);
  html = html.replace('</style>', '.yt-stealth{position:fixed;left:-10000px;top:0;width:240px;height:240px;overflow:hidden;pointer-events:none;opacity:.01}.song-play{font-size:20px;padding:19px 16px}.yt-reveal{margin-top:14px}.yt-reveal iframe{display:block;width:100%;aspect-ratio:16/9;border:0;border-radius:14px}.mix-note{margin-top:8px;color:var(--muted);font-size:11px;text-align:center}</style>');
  html = replaceRequired(html, "var ytPlayer=null,ytApiReady=!!(window.YT&&window.YT.Player),ytPlayerReady=false,pendingSong=null,ytScriptLoading=false;", "var ytPlayer=null,ytApiReady=!!(window.YT&&window.YT.Player),ytPlayerReady=false,pendingSong=null,ytScriptLoading=false,pendingSync=null,syncTimer=null;", 'song vars');
  html = html.replace(/function songHostHtml\(cur\)\{[\s\S]*?\n\}\nfunction songPlayerHtml\(cur\)\{[\s\S]*?\n\}/, `function songHostHtml(cur){
...
function songPlayerHtml(cur){
  ...
  html = html.replace(/function createYTPlayer\(cur\)\{[\s\S]*?\n\}\nfunction playSnippet\(stage\)\{[\s\S]*?\n\}\nfunction bind\(\)\{/, `function createYTPlayer(cur){
  if(!document.getElementById('ytPlayer')||ytPlayer)return;
  try{ytPlayer=new YT.Player('ytPlayer',{height:'240',width:'240',videoId:cur.videoId,host:'https://www.youtube-nocookie.com',playerVars:{controls:0,playsinline:1,autoplay:0,rel:0,origin:location.origin},events:{onReady:function(){ytPlayerReady=true;try{ytPlayer.cueVideoById({videoId:cur.videoId,startSeconds:Number(cur.startSeconds||0)});}catch(e){}if(pendingSync){var p=pendingSync;pendingSync=null;playSyncedSong(p);}},onError:function(){ytPlayerReady=false;toast('Dieser Song kann hier nicht abgespielt werden.');}}});}catch(e){toast('Song konnte nicht geladen werden.');}
}
function playSyncedSong(m){
  ...
  syncTimer=setTimeout(function(){if(!state||!state.current||state.current.type!=='song')return;var late=Math.max(0,(Date.now()-Number(m.at||Date.now()))/1000);var start=Number(m.startSeconds||0)+late;try{if(ytPlayer.unMute)ytPlayer.unMute();if(ytPlayer.setVolume)ytPlayer.setVolume(100);if(ytPlayer.seekTo)ytPlayer.seekTo(start,true);if(ytPlayer.playVideo)ytPlayer.playVideo();}catch(e){toast('Song konnte auf diesem Gerät nicht gestartet werden.');}},delay);
}
function playSongButton(){var cur=state&&state.current;if(!cur||cur.type!=='song'||!cur.isHost)return;send({t:'songPlay'});}
function bind(){`);
  html = replaceRequired(html, "    if(m.t==='error'){toast(m.m||'Fehler');return;}\n    if(m.t==='guessFeedback'||m.t==='personFeedback'){", "    if(m.t==='error'){toast(m.m||'Fehler');return;}\n    if(m.t==='songPlay'){playSyncedSong(m);return;}\n    if(m.t==='guessFeedback'||m.t==='personFeedback'){", 'song sync message');
  html = replaceRequired(html, "  var load=document.getElementById('loadYT');if(load)load.onclick=function(){ensureYTApi(state.current);};\n  var broken=document.getElementById('songBroken');if(broken)broken.onclick=function(){send({t:'songBroken'});};\n  a=document.querySelectorAll('.snippet');for(i=0;i<a.length;i++)a[i].onclick=function(){playSnippet(Number(this.getAttribute('data-stage')));};", "  var play=document.getElementById('songPlay');if(play)play.onclick=playSongButton;\n  var broken=document.getElementById('songBroken');if(broken)broken.onclick=function(){send({t:'songBroken'});};", 'single song button');
  html = replaceRequired(html, "  if(r.votes&&r.votes.length){", "  if(r.type==='song'&&r.videoId)out+='<div class=\"yt-reveal\"><iframe src=\"https://www.youtube.com/embed/'+esc(r.videoId)+'?autoplay=1&playsinline=1&rel=0\" title=\"Musikvideo\" allow=\"autoplay; encrypted-media; picture-in-picture\" allowfullscreen></iframe></div>';\n  if(r.votes&&r.votes.length){", 'result video');
  html = replaceRequired(html, "  var songHost=state&&state.phase==='question'&&state.current&&state.current.type==='song'&&state.current.isHost;\n  if(!songHost&&ytPlayer)cleanupYouTube();", "  var songActive=state&&state.phase==='question'&&state.current&&state.current.type==='song';\n  if(!songActive&&ytPlayer)cleanupYouTube();", 'song cleanup');
  html = replaceRequired(html, "  bind();tick();timerLoop=setInterval(tick,250);", "  bind();if(songActive&&!ytPlayer)setTimeout(function(){if(state&&state.current&&state.current.type==='song')ensureYTApi(state.current);},0);tick();timerLoop=setInterval(tick,250);", 'song preload');
  html = html.replace('<div class="footer">Automatischer Mix · keine Einstellungen · 18+</div>', '<div class="footer">Automatischer Mix · 18+</div><div class="mix-note">10-Runden-Mix: Schätzen 2× · Quiz · Person · Bild · Song · Social · Entweder/Oder · 18+ · Gruppe</div>');
  html = html.replace('</style>', '.cat-tools{display:flex;gap:8px;margin-bottom:9px}.cat-tools .mini{flex:1}.cat-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.cat-toggle,.cat-view{min-height:46px;border-radius:13px;padding:10px 11px;border:1px solid var(--line);background:#15101e;color:#8d8298;font-weight:850;font-size:12px;text-align:left}.cat-toggle{cursor:pointer}.cat-toggle.active,.cat-view.active{color:#11170a;background:var(--accent);border-color:var(--accent)}.cat-view{opacity:.85}.cat-help{color:var(--muted);font-size:11px;line-height:1.4;margin:8px 1px 12px}@media(max-width:380px){.cat-grid{grid-template-columns:1fr}}</style>');
  html = html.replace(/function lobbyHtml\(\)\{[\s\S]*?\n\}\nfunction targetName/, "function categoryPickerHtml(){\n  var selected=state.selectedCats||Object.keys(state.cats||{}),host=state.you===state.hostId,out='';\n  if(host)out+='<div class=\\\"cat-tools\\\"><button id=\\\"allCats\\\" class=\\\"mini\\\">Alle auswählen</button><span class=\\\"mini\\\" style=\\\"text-align:center\\\">'+selected.length+' aktiv</span></div>';\n  out+='<div class=\\\"cat-grid\\\">';\n  for(var key in state.cats){var active=selected.indexOf(key)>=0,cls=host?'cat-toggle':'cat-view';out+='<button '+(host?'':'disabled')+' class=\\\"'+cls+(active?' active':'')+'\\\" data-cat=\\\"'+esc(key)+'\\\">'+(active?'✓ ':'')+esc(state.cats[key])+'</button>';}\n  out+='</div><div class=\\\"cat-help\\\">'+(host?'Tippe Kategorien an oder aus. Mindestens eine bleibt aktiv.':'Der Host hat diese Kategorien für die Runde ausgewählt.')+'</div>';\n  return out;\n}\nfunction lobbyHtml(){\n  var host=state.you===state.hostId;\n  var controls=host?'<div class=\\\"grid2\\\"><button id=\\\"start\\\" class=\\\"btn primary\\\">Spiel starten</button><button id=\\\"leaveLobby\\\" class=\\\"btn ghost\\\">Lobby verlassen</button></div>':'<div class=\\\"waiting\\\">Der Host startet das Spiel.</div><button id=\\\"leaveLobby\\\" class=\\\"btn ghost\\\">Lobby verlassen</button>';\n  return topHtml()+'<div class=\\\"card center\\\"><div class=\\\"meta\\\">Spielcode</div><div class=\\\"big-code\\\">'+esc(state.code)+'</div><div class=\\\"note\\\">Mitspieler öffnen dieselbe Seite und geben diesen Code ein.</div></div><div class=\\\"section\\\">Kategorien</div>'+categoryPickerHtml()+'<div class=\\\"section\\\">Spieler</div>'+playersHtml()+'<div style=\\\"height:12px\\\"></div>'+controls+'<div class=\\\"footer\\\">Eigener Kategorienmix · 18+</div>';\n}\nfunction targetName");
  html = html.replace("  var leave=document.getElementById('leaveLobby');if(leave)leave.onclick=function(){send({t:'leave'});clearJoin();state=null;feedback=null;render();};", "  var leave=document.getElementById('leaveLobby');if(leave)leave.onclick=function(){send({t:'leave'});clearJoin();state=null;feedback=null;render();};\n  var allCats=document.getElementById('allCats');if(allCats)allCats.onclick=function(){send({t:'allCats'});};\n  var catBtns=document.querySelectorAll('.cat-toggle');for(i=0;i<catBtns.length;i++)catBtns[i].onclick=function(){send({t:'toggleCat',cat:this.getAttribute('data-cat')});};");
...
wss.on('connection', (ws) => {
  let room = null; let me = null; let msgWindowAt = Date.now(); let msgCount = 0;
  const error = (m) => send(ws, { t: 'error', m });
  ws.on('message', (raw) => {
    if (raw.length > 8192) return;
    const now = Date.now(); if (now - msgWindowAt > 10000) { msgWindowAt = now; msgCount = 0; }
    msgCount += 1; if (msgCount > 200) return;
    let msg; try { msg = JSON.parse(String(raw)); } catch { return; }
    if (msg.t === 'ping') return send(ws, { t: 'pong' });
    if (msg.t === 'create') {
      if (room || me) return error('Du bist bereits in einer Lobby.');
      room = createRoom(); me = uid(); room.players[me] = { name: cleanName(msg.name, 'Host'), drinks: 0, connected: true, ws }; room.order.push(me); room.hostId = me;
      send(ws, { t: 'joined', code: room.code, id: me }); return broadcast(room);
    }
    if (msg.t === 'join') {
      if (room || me) return error('Du bist bereits in einer Lobby.');
      const code = String(msg.code || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5); const target = rooms.get(code);
      if (!target) return error('Diesen Spielcode gibt es nicht.'); if (connectedIds(target).length >= 30) return error('Diese Lobby ist voll (max. 30 Spieler).');
      room = target; me = uid(); room.players[me] = { name: cleanName(msg.name), drinks: 0, connected: true, ws }; room.order.push(me);
      send(ws, { t: 'joined', code: room.code, id: me }); return broadcast(room);
    }
    if (msg.t === 'rejoin') {
      if (room || me) return error('Du bist bereits in einer Lobby.');
      const target = rooms.get(String(msg.code || '').toUpperCase()); if (!target || !target.players[msg.id]) return send(ws, { t: 'reset' });
      room = target; me = msg.id; const previousWs = room.players[me].ws; room.players[me].connected = true; room.players[me].ws = ws;
      if (!room.hostId) room.hostId = me;
    ...
    if (msg.t === 'toggleCat' && isHost && (room.phase === 'lobby' || room.phase === 'end')) {
      const cat = String(msg.cat || ''); if (!CATEGORY_ORDER.includes(cat) || !Array.isArray(Q[cat]) || !Q[cat].length) return;
      const selected = Array.isArray(room.selectedCats) && room.selectedCats.length ? [...room.selectedCats] : [...CATEGORY_ORDER]; const idx = selected.indexOf(cat);
      if (idx >= 0) { if (selected.length <= 1) return error('Mindestens eine Kategorie muss aktiv bleiben.'); selected.splice(idx, 1); } else selected.push(cat);
      room.selectedCats = CATEGORY_ORDER.filter((x) => selected.includes(x)); room.deck = []; room.lastCat = null; return broadcast(room);
    }
    ...
    if (msg.t === 'start' && isHost && (room.phase === 'lobby' || room.phase === 'end')) { if (!connectedIds(room).length) return error('Niemand ist im Raum.'); startRound(room).catch(() => error('Runde konnte nicht gestartet werden.')); return; }
    if (msg.t === 'next' && isHost && room.phase === 'results') { startRound(room).catch(() => error('Runde konnte nicht gestartet werden.')); return; }
    if (msg.t === 'end' && isHost) { clearTimers(room); room.phase = 'end'; room.current = null; return broadcast(room); }
    if (msg.t === 'host' && isHost && room.phase === 'lobby' && room.players[msg.id]?.connected) { room.hostId = msg.id; return broadcast(room); }
    if (msg.t === 'leave') {
      const oldRoom = room; const oldId = me; const wasHost = oldRoom.hostId === oldId; send(ws, { t: 'reset' });
      delete oldRoom.players[oldId]; oldRoom.order = oldRoom.order.filter((id) => id !== oldId); if (wasHost) oldRoom.hostId = connectedIds(oldRoom)[0] || null; room = null; me = null;
  ...
  ws.on('close', () => {
    if (!room || !me || !room.players[me]) return; if (room.players[me].ws !== ws) return;
    const oldRoom = room; leaveRoom(oldRoom, me);
    setTimeout(() => { if (!rooms.has(oldRoom.code)) return; if (!connectedIds(oldRoom).length) { clearTimers(oldRoom); rooms.delete(oldRoom.code); } }, 30 * 60 * 1000).unref?.();
  });
});
function shutdown() {
  for (const room of rooms.values()) clearTimers(room);
  wss.close(() => {}); server.close(() => process.exit(0)); setTimeout(() => process.exit(0), 1500).unref();
}
