...
// Change the frontend module directly in the v4.7 generator source.
src = replaceRequired(
  src,
...
// Everything below targets the generated production server, not server-v461.js itself.
// Append these base patches after v4.7's own patches so each layer sees the expected source.
const patches = [];

patches.push(basePatchSource(
...
patches.push(basePatchSource(numericStroke, rgbStroke, 'RGB Zeichnen'));
patches.push(basePatchSource(numericStroke, rgbStroke, 'RGB Alle-malen'));
...
patches.push(basePatchSource(oldClampStroke, newClampStroke, 'RGB Farbwert Zeichnen'));
patches.push(basePatchSource(oldClampStroke, newClampStroke, 'RGB Farbwert Alle-malen'));
...
patches.push(basePatchSource("broadcast(room); setTimeout(tick, 55).unref?.();", "broadcast(room); setTimeout(tick, 40).unref?.();", 'Pong Tickrate'));
patches.push(basePatchSource("setTimeout(tick, 250).unref?.();", "setTimeout(tick, 150).unref?.();", 'Pong Start'));

const runtimeAnchor = "const runtimeAnchor = 'const runtime = new Module(basePath, module.parent || module);';";
src = replaceRequired(src, runtimeAnchor, patches.join('\n') + '\n\n' + runtimeAnchor, 'v4.7.1 Generator-Patches');
