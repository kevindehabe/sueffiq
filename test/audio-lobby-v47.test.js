...
test('Zeitgefühl stays a minigame while logo guessing is a normal interactive category',async()=>{const html=await(await fetch(`${HTTP}/`)).text();assert.match(html,/blindTimerAction/);assert.match(html,/miniTimerStart/);assert.match(html,/miniTimerStop/);assert.match(html,/logoGameImage/);assert.match(html,/Marke eingeben/);assert.match(html,/logoBroken/);assert.match(html,/Erkenne das Logo/);assert.match(html,/goMainMenu/);assert.match(html,/Während er läuft siehst du keine Zeit/);assert.doesNotMatch(html,/miniLogoGuess/);});
