...
test.before(async()=>{child=spawn(process.execPath,['server-v3.js'],{cwd:path.join(__dirname,'..'),env:{...process.env,PORT:String(PORT),CI:'true'},stdio:['ignore','ignore','pipe']});const h=await waitHealth();assert.equal(h.ok,true);assert.equal(h.version,'4.7.0');assert.equal(h.songSource,'youtube');assert.equal(h.youtubePlayback,true);assert.equal(h.ruleRounds,false);assert.equal(h.logoCategory,true);});
...
test('Zeitgefühl stays a minigame while logo guessing is a normal interactive category',async()=>{const html=await(await fetch(`${HTTP}/`)).text();assert.match(html,/blindTimerAction/);assert.match(html,/miniTimerStart/);assert.match(html,/miniTimerStop/);assert.match(html,/logoGameImage/);assert.match(html,/Marke eingeben/);assert.match(html,/logoBroken/);assert.match(html,/function logoRoundHtml/);assert.match(html,/t==='logo'/);assert.match(html,/goMainMenu/);assert.match(html,/Während er läuft siehst du keine Zeit/);assert.doesNotMatch(html,/miniLogoGuess/);});
