...
test('rules start pseudo-randomly, stay room-wide and expire after at most ten rounds',async()=>{const html=await(await fetch(`${HTTP}/`)).text();assert.match(html,/PARTY_RULE_MAX_ROUNDS=10/);assert.match(html,/PARTY_RULE_CHANCE=28/);assert.match(html,/partyRuleStarts/);assert.match(html,/regel-start/);assert.match(html,/r-PARTY_RULE_MAX_ROUNDS\+1/);assert.doesNotMatch(html,/PARTY_EVENTS=\[/);assert.doesNotMatch(html,/currentPartyEvent/);assert.doesNotMatch(html,/⚡ EVENT/);assert.doesNotMatch(html,/x\+=3/);});
