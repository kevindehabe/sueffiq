...
test('party events are deterministic room-wide events and include a water break',async()=>{const html=await(await fetch(`${HTTP}/`)).text();assert.match(html,/PARTY_EVENTS=\[/);assert.match(html,/currentPartyEvent/);assert.match(html,/Wasser-Event/);assert.match(html,/String\(s\.code\)\+'\|event\|'/);assert.match(html,/⚡ EVENT/);});
