...
test('person guess marks common typos as near and unrelated guesses as wrong', () => {
  const person = { name: 'Cristiano Ronaldo', aliases: ['CR7'] };
  assert.equal(matchPersonGuess(person, 'Ronaldoo').status, 'near');
  assert.equal(matchPersonGuess(person, 'Messi').status, 'wrong');

  const britney = { name: 'Britney Spears', aliases: [] };
  assert.equal(matchPersonGuess(britney, 'Brintey').status, 'near');
});

test('wer würde eher is capped at three sips even with many votes', () => {
  const votes = { a: 'x', b: 'x', c: 'x', d: 'x', e: 'x', f: 'x', g: 'y' };
  assert.deepEqual(voteSipMap(votes).sips, { x: 3, y: 1 });
});

test('estimate ties for closest stay dry and farther guesses get more sips with a max of three', () => {
  ...
  assert.ok(Math.max(...rows.map((r) => r.sips)) <= 3);
});
...
test('majority punishes less popular choices by vote gap without exceeding three', () => {
  ...
  assert.ok(Math.max(0, ...Object.values(out.sipsByPlayer)) <= 3);
});
...
test('scale keeps values around the median dry', () => {
  ...
  assert.ok(byId.a > 0);
  assert.ok(byId.d > 0);
  assert.ok(Math.max(...out.rows.map((r) => r.sips)) <= 3);
});
