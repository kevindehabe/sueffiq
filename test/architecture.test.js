...
test('production stays one process without child servers or temp runtimes', () => {
  ...
  const youtubeTimerExtension = fs.readFileSync(path.join(root, 'server-v51.js'), 'utf8');
  ...
  const production = `${entry}\n${youtubeTimerExtension}\n${audioExtension}\n${tapExtension}\n${extension}\n${core}`;

  assert.equal(pkg.main, 'server.js');
  assert.equal(pkg.scripts.start, 'node server.js');
  assert.match(compat, /require\('\.\/server'\)/);
  assert.match(entry, /require\('\.\/server-v51'\)/);
  assert.match(youtubeTimerExtension, /server-v471/);
