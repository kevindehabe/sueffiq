...
function buildRuntimeCore() {
  ...
  // Songrunde: Host darf mitraten. Die Runde endet erst, wenn alle richtig haben oder die Zeit ausläuft.
  core = core.replace(
  ...
  // Die Untertypen Nie/Wahl, Wahrheit/Pflicht und Mehrheit/Skala wechseln blockweise.
  core = core.replace(
...
function patchedHtml() {
  ...
  return '<div id="songHostPanel"><div class="dj"><div class="dj-title">🎵 Errate den Song</div><div class="meta">Kein Titel, kein Cover und kein Video vor der Auflösung. Du kannst als Host selbst mitraten.</div></div>'+ '<button id="songPlay" class="btn primary song-play">▶ Song abspielen</button>'+ '<div id="ytStealth" class="yt-stealth" aria-hidden="true"><div id="ytPlayer"></div></div>'+ '<div style="height:10px"></div>'+guessHtml(cur,'Songtitel eingeben …')+'<button id="songBroken" class="btn ghost" style="margin-top:10px">Song funktioniert nicht · Runde überspringen</button></div>';
}
function songPlayerHtml`
  );
  ...
  try{
    ytPlayer=new YT.Player('ytPlayer',{
      ...
      events:{
        onReady:function(){ytPlayerReady=true;if(pendingPlayStage!==null){var s=pendingPlayStage;pendingPlayStage=null;playSnippet(s);}},
...
function playSnippet(stage){
  ...
  try{ytPlayer.loadVideoById({videoId:cur.videoId,startSeconds:start,endSeconds:start+seconds});if(ytPlayer.unMute)ytPlayer.unMute();}
...
function playSongButton(){
  ...
  var stage=Math.min(songPlayCount,3);songPlayCount=Math.min(songPlayCount+1,3);
  if(!ytPlayer||!ytPlayerReady){pendingPlayStage=stage;ensureYTApi(cur);return;}
  playSnippet(stage);
}
function bind(){`
