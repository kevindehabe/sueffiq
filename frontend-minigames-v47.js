...
function setupAllDraw(){
  var cur=state&&state.current;if(!cur||cur.miniType!=='allemalen')return;if(cur.miniStage==='rank'){setupAllDrawRanking(cur);return;}
  drawCanvas=document.getElementById('drawCanvas');if(!drawCanvas)return;drawCtx=prepCanvas(drawCanvas,cur.strokes||[]);
  var colors=document.querySelectorAll('.draw-color');for(var i=0;i<colors.length;i++)colors[i].onclick=function(){if(cur.drawDone)return;drawColor=Number(this.getAttribute('data-c'))||0;for(var j=0;j<colors.length;j++)colors[j].classList.toggle('active',colors[j]===this);};
  var clear=document.getElementById('allDrawClear');if(clear)clear.onclick=function(){if(cur.drawDone)return;clearDrawCanvas();send({t:'allDrawClear'});};var done=document.getElementById('allDrawDone');if(done)done.onclick=function(){done.disabled=true;done.textContent='Fertig ✓';send({t:'allDrawDone'});};if(cur.drawDone)return;
  ...
  function different(a,b){return !!(a&&b&&(Math.abs(a.x-b.x)>.0001||Math.abs(a.y-b.y)>.0001));}
  function sendPending(p){if(!drawSentPoint||!p)return;send({t:'allDrawStroke',s:[drawSentPoint.x,drawSentPoint.y,p.x,p.y,drawColor]});drawSentPoint=p;drawLastSent=Date.now();}
  drawCanvas.onpointerdown=function(e){e.preventDefault();try{drawCanvas.setPointerCapture(e.pointerId);}catch(x){}drawLast=pos(e);drawSentPoint=drawLast;drawLastSent=0;};
  drawCanvas.onpointermove=function(e){if(!drawLast)return;e.preventDefault();var p=pos(e),local=[drawLast.x,drawLast.y,p.x,p.y,drawColor];paintStroke(local);var now=Date.now();if(now-drawLastSent>=60)sendPending(p);drawLast=p;};
  drawCanvas.onpointerup=function(e){if(!drawLast)return;e.preventDefault();var p=pos(e);if(different(drawLast,p))paintStroke([drawLast.x,drawLast.y,p.x,p.y,drawColor]);if(different(drawSentPoint,p))sendPending(p);drawLast=null;drawSentPoint=null;};
  drawCanvas.onpointercancel=function(){if(different(drawSentPoint,drawLast))sendPending(drawLast);drawLast=null;drawSentPoint=null;};
}
